<template>
  <a-layout class="certificate-edit-layout">
    <!-- 使用 HeaderComponent -->
    ...
    <a-layout>
      ...
      <a-layout-content class="content">
        ...
        <!-- 证书编辑表单 -->
        <a-card class="form-card form-container">
          <a-form ref="certificateFormRef" class="form" :model="certificateForm" :rules="rules"
            :label-col="{ style: { width: '120px' } }" :wrapper-col="{ span: 12 }" v-if="!loading">
                  ...
                  <div class="upload-item">
                    ...
                    <div class="upload-info">
                      ...
                      <div class="upload-status">
                        ...
                        <close-circle-outlined v-else-if="file.status === 'error'" style="color: #f5222d" />
                        <a v-if="file.status !== 'uploading'" @click="handleRemove(file)">删除</a>
                      </div>
                    </div>
                  </div>
                </template>
...
<!--                  </div>-->
            ...
            <a-form-item>
              <a-space>
                <a-button type="primary" @click="handleSubmit" :disabled="!hasEditPermission"
                  v-if="hasEditPermission">保存修改</a-button>
                <a-tooltip v-else title="您没有编辑证书的权限">
                  <a-button type="primary" disabled>保存修改</a-button>
                </a-tooltip>
...
import { getCertificateDetail, updateCertificate } from '@/api/certificate';
  ...
  setup() {
    ...
    const fetchCertificateDetail = async () => {
      ...
      try {
        ...
        if (res.code === 200 && res.data) {
          // 填充表单数据
          certificateForm.id = res.data.id;
          certificateForm.name = res.data.name;
          certificateForm.description = res.data.description;
          certificateForm.templateUrl = res.data.templateUrl;
          certificateForm.expireTime = res.data.expireTime;

          // 设置过期时间
          if (res.data.expireTime) {
            expireDate.value = dayjs(res.data.expireTime);
          }
    ...
    const handleSubmit = async () => {
      try {
        ...
        // 提交表单
        const res = await updateCertificate(certificateForm);
        if (res.code === 200) {
          message.success('更新证书成功');
          router.push('/certificate/list');
        } else {
          message.error(res.message || '更新证书失败');
        }
      } catch (error) {
    ...
    onMounted(() => {
      ...
      getUserInfo();
      // 检查是否有编辑证书的权限
      hasEditPermission.value = hasPermission('certificate:edit');
      if (!hasEditPermission.value) {
        message.warning('您没有编辑证书的权限，部分功能将被禁用');
      }
      // 获取证书详情
