<template>
  <div>
    <a-modal
      ...
      @cancel="handleCancel"
    >
      <div class="certificate-relate">
        <a-spin :spinning="loading">

          <div class="filter-container">
            <span class="type-label">类型:</span><a-checkbox-group v-model:value="selectedTypes" @change="handleFilterChange">
...
import { getCertificateDetail, getCertificateRelatedContent } from '@/api/certificate';

export default defineComponent({
  name: 'CertificateRelateComponent',
  props: {
    certificateId: {
      type: Number,
      required: true
  ...
  setup(props, { emit }) {
    ...
    const certificateName = ref(''); // 存储当前证书名称

    // 模态框标题
    const modalTitle = computed(() => {
      return `查看关联项：${certificateName.value}`;
    });
    ...
    const fetchRelateContent = async () => {
      ...
      try {
        // 获取证书关联内容
        const relateRes = await getCertificateRelatedContent({bizId:props.certificateId, type:'CERTIFICATE'});
        ...
        // 获取证书详情，获取证书名称
        const detailRes = await getCertificateDetail(props.certificateId);
        if (detailRes.code === 200 && detailRes.data) {
          certificateName.value = detailRes.data.name || '';
        } else {
          console.error('获取证书名称失败');
          certificateName.value = '';
        }
      } catch (error) {
        console.error('获取证书关联内容失败:', error);
      } finally {
        loading.value = false;
      }
    };
